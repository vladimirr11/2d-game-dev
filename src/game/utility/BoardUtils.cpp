...
bool BoardUtils::isInsideBoard(const Point& absPos) {
    const Rectangle boardBounds{FIRST_TILE_X_POS, FIRST_TILE_Y_POS, BOARD_SIZE * TILE_SIZE,
                                BOARD_SIZE * TILE_SIZE};

    return boardBounds.isPointInsideRect(absPos);
}

int32_t BoardUtils::getOpponentId(const int32_t activePlayerId) {
...
BoardPosition BoardUtils::getAdjacentPos(const Direction dir, const BoardPosition& currPos) {
    ...
    default:
        std::cerr << "BoardUtils::getAdjacentPos() recieved invalid directory - " << dir
                  << std::endl;
        break;
    }
...
bool BoardUtils::checkCollision(const BoardPosition& selectedPos,
...
TileType BoardUtils::getTileType(const BoardPosition& boardPos,
    ...
    if (checkCollision(boardPos, playerPieces, collisionIdx)) {
        return TileType::GUARD;
    }

    if (checkCollision(boardPos, enemyPieces, collisionIdx)) {
